...
function colorCondition(desc) {
  ...
  if (desc.includes("thunderstorm"))
    return colors.magenta + "⚡ " + desc + colors.reset;
  return desc;
}
    ...
    .get(url, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          console.error("❌ Failed to parse current weather data", err.message);
        }
      });
    })
        ...
        try {
          ...
          Object.keys(daily)
            ...
            .forEach((date) => {
              ...
              console.log(
                `🌡️ Min Temp: ${colorTemp(minTemp)} | Max Temp: ${colorTemp(
                  maxTemp
                )} | Avg Temp: ${colorTemp(avgTemp)}`
              );
              console.log(
                `💨 Wind: ${colorWind(avgWind)} | 🌧️ Rain: ${colorRain(
                  totalRain
                )} | ☁️ Condition: ${colorCondition(desc)}`
              );
              console.log("\n");
            });
        } catch (err) {
          console.error("❌ Failed to parse forecast data", err.message);
        }
      });
    })
    .on("error", (err) => console.error("❌ Request error:", err.message));
...
function getCurrentCity(callback) {
  https
    .get("https://ipwhois.app/json/", (res) => {
      let data = "";
      res.on("data", (chunk) => (data += chunk));
      res.on("end", () => {
        try {
    ...
    .on("error", () => callback("London"));
}

let city = process.argv[2];
let days = process.argv[3] ? parseInt(process.argv[3]) : undefined;
